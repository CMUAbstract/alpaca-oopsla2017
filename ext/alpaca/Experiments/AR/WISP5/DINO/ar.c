...
/*DINO Task Names*/
#define LOOPTOP_TASK 0
#define SAMPLE_TASK 1
#define PROCESS_TASK 2
#define STATS_TASK 3
#define INIT_TASK 4

DINO_RECOVERY_ROUTINE_LIST_BEGIN()

  DINO_RECOVERY_RTN_BEGIN(LOOPTOP_TASK)
  DINO_RECOVERY_RTN_END()

  DINO_RECOVERY_RTN_BEGIN(SAMPLE_TASK)
  DINO_RECOVERY_RTN_END()
  
  DINO_RECOVERY_RTN_BEGIN(PROCESS_TASK)
  DINO_RECOVERY_RTN_END()
  
  DINO_RECOVERY_RTN_BEGIN(STATS_TASK)
  DINO_RECOVERY_RTN_END()
...
volatile __fram unsigned int __NV_movingCount;
volatile __fram unsigned int __NV_stationaryCount;
volatile __fram unsigned int __NV_totalCount;
volatile __fram float __NV_movingPct;
volatile __fram float __NV_stationaryPct;

volatile __fram unsigned __NV_numBoots = 0;
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    DINO_TASK_BOUNDARY(LOOPTOP_TASK,NULL);
    ...
    getNextSample();
    DINO_TASK_BOUNDARY(PROCESS_TASK,NULL);
    featurize();
    ...
    int class = classify();
    DINO_TASK_BOUNDARY(STATS_TASK,NULL);
