...
/* cache of saved readings */
#define READINGS_TO_SAVE 5000
static __fram threeAxis_t readings[READINGS_TO_SAVE];
static unsigned numSavedReadings = 0;
...
void getOneSample(accelReading tr) {
  ...
  if (numSavedReadings < READINGS_TO_SAVE) {
    readings[numSavedReadings].x = threeAxis.x;
    readings[numSavedReadings].y = threeAxis.y;
    readings[numSavedReadings].z = threeAxis.z;
    numSavedReadings++;
  }
