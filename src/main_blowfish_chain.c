...
struct msg_s {
    ...
    CHAN_FIELD_ARRAY(uint32_t, s2, 256);
    CHAN_FIELD_ARRAY(uint32_t, s3, 256);
};
struct msg_key {
    CHAN_FIELD_ARRAY(uint32_t, key, 18);
};
struct msg_ukey {
...
struct msg_index2 {
    CHAN_FIELD(uint32_t, index2);
};
struct msg_self_index2 {
    SELF_CHAN_FIELD(uint32_t, index2);
};
...
struct msg_self_input {
	SELF_CHAN_FIELD_ARRAY(uint32_t, input, 2);
};
#define FIELD_INIT_msg_self_input { \
	SELF_FIELD_ARRAY_INITIALIZER(2) \
}
struct msg_input {
	CHAN_FIELD_ARRAY(uint32_t, input, 2);
...
void task_init() {
	...
	CHAN_OUT1(uint32_t, index2, lzero, MC_OUT_CH(ch_index2, task_init, task_set_key2, task_start_encrypt3));
...
void task_init_key() {
	unsigned i;
	for (i = 0; i < 18; ++i) {
		CHAN_OUT1(uint32_t, key[i], init_key[i], CH(task_init_key, task_set_key));
	}
...
void task_set_key() {
	...
	for (i = 0; i < 18; ++i) {
		...
		key = *CHAN_IN1(uint32_t, key[i], CH(task_init_key, task_set_key));
#if VERBOSE > 0
		print_long(key);
		print_long(ri);
#endif
		key ^= ri;
		CHAN_OUT1(uint32_t, key[i], key, CH(task_set_key, task_encrypt));
#if VERBOSE > 0
		print_long(key);
#endif
	}
...
void task_set_key2() {
	uint32_t zero = 0L;
	uint32_t index2 = *CHAN_IN2(uint32_t, index2, MC_IN_CH(ch_index2, task_init, task_set_key2), SELF_IN_CH(task_set_key2));
	uint32_t k0, k1;
	if (index2 == 0) {
		CHAN_OUT1(uint32_t, input[0], zero, CH(task_set_key2, task_encrypt));
		CHAN_OUT1(uint32_t, input[1], zero, CH(task_set_key2, task_encrypt));
		index2 += 2;
		CHAN_OUT1(uint32_t, index2, index2, SELF_OUT_CH(task_set_key2));
...
//		WRITE(return_to, GV(return_to));
		...
		if (index2 < 20) { //set key
			k0 = *CHAN_IN1(uint32_t, input[0], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
			k1 = *CHAN_IN1(uint32_t, input[1], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
			CHAN_OUT1(uint32_t, key[index2-2], k0, CH(task_set_key2, task_encrypt));
			CHAN_OUT1(uint32_t, key[index2-1], k1, CH(task_set_key2, task_encrypt));
#if VERBOSE > 0
			LOG("key[%u]=",index2-2);
			print_long(k0);
			LOG("key[%u]=",index2-1);
			...
			if (index2 < (256 + 20)) { //set s0 
				k0 = *CHAN_IN1(uint32_t, input[0], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				k1 = *CHAN_IN1(uint32_t, input[1], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				CHAN_OUT1(uint32_t, s0[index2-20], k0, CH(task_set_key2, task_encrypt));
				CHAN_OUT1(uint32_t, s0[index2-19], k1, CH(task_set_key2, task_encrypt));
#if VERBOSE > 0
				if (index2 == 20 || index2 == 254 + 20) {
					LOG("s0[%u]=",index2-20);
					print_long(k0);
					LOG("s0[%u]=",index2-19);
					print_long(k1);	
			...
			else if (index2 < (512 + 20)) { //set s1
				k0 = *CHAN_IN1(uint32_t, input[0], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				k1 = *CHAN_IN1(uint32_t, input[1], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				CHAN_OUT1(uint32_t, s1[index2-(256+20)], k0, CH(task_set_key2, task_encrypt));
				CHAN_OUT1(uint32_t, s1[index2-(256+19)], k1, CH(task_set_key2, task_encrypt));
#if VERBOSE > 0
				if (index2 == 256 + 20 || index2 == (256*2-2) + 20) {
					LOG("s1[%u]=",index2-(256+20));
					print_long(k0);
					LOG("s1[%u]=",index2-(256+19));
					print_long(k1);	
				}
#endif
				index2 += 2;
			...
			else if (index2 < (256*3 + 20)) { //set s2
				k0 = *CHAN_IN1(uint32_t, input[0], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				k1 = *CHAN_IN1(uint32_t, input[1], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				CHAN_OUT1(uint32_t, s2[index2-(256*2+20)], k0, CH(task_set_key2, task_encrypt));
				CHAN_OUT1(uint32_t, s2[index2-(256*2+19)], k1, CH(task_set_key2, task_encrypt));
#if VERBOSE > 0
				if (index2 == 256*2 + 20 || index2 == (256*3-2) + 20) {
					LOG("s2[%u]=",index2-(256*2+20));
					print_long(k0);
					LOG("s2[%u]=",index2-(256*2+19));
					print_long(k1);	
				}
#endif
				index2 += 2;
			...
			else if (index2 < (256*4 + 20)) {
				k0 = *CHAN_IN1(uint32_t, input[0], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				k1 = *CHAN_IN1(uint32_t, input[1], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
				CHAN_OUT1(uint32_t, s3[index2-(256*3+20)], k0, CH(task_set_key2, task_encrypt));
				CHAN_OUT1(uint32_t, s3[index2-(256*3+19)], k1, CH(task_set_key2, task_encrypt));
#if VERBOSE > 0
				if (index2 == 256*3 + 20 || index2 == (256*4-2) + 20) {
					LOG("s3[%u]=",index2-(256*3+20));
					print_long(k0);
					LOG("s3[%u]=",index2-(256*3+19));
					print_long(k1);	
				}
#endif
				index2 += 2;
...
//	LOG("return_to: %u\r\n", return_to);
//	uint32_t test = *READ(return_to, 0);
//	LOG("test: ");
//	print_long(test);
...
//	print_long(test);
	...
	r = *CHAN_IN3(uint32_t, input[0], SELF_CH(task_encrypt),CH(task_set_key2, task_encrypt), CH(task_start_encrypt, task_encrypt));
	l = *CHAN_IN3(uint32_t, input[1], SELF_CH(task_encrypt),CH(task_set_key2, task_encrypt), CH(task_start_encrypt, task_encrypt));
	for (index = 0; index < 17; ++index) {
		p = *CHAN_IN2(uint32_t, key[index], CH(task_set_key, task_encrypt), CH(task_set_key2, task_encrypt));
#if VERBOSE > 0
		if (test){
			print_long(p);
			print_long(r);
...
#endif
		s0 = *CHAN_IN2(uint32_t, s0[(r>>24L)], CH(task_init_s, task_encrypt), CH(task_set_key2, task_encrypt));
		s1 = *CHAN_IN2(uint32_t, s1[((r>>16L)&0xff)], CH(task_init_s, task_encrypt), CH(task_set_key2, task_encrypt));
		s2 = *CHAN_IN2(uint32_t, s2[((r>> 8L)&0xff)], CH(task_init_s, task_encrypt), CH(task_set_key2, task_encrypt));
		s3 = *CHAN_IN2(uint32_t, s3[((r     )&0xff)], CH(task_init_s, task_encrypt), CH(task_set_key2, task_encrypt));
#if VERBOSE > 0
		if (test){
			print_long((r));
			print_long((r>>4L));
			print_long((r>>24L));
			print_long(s0);
			s0 = *CHAN_IN2(uint32_t, s0[54], CH(task_init_s, task_encrypt), CH(task_set_key2, task_encrypt));
			print_long(s0);
		}
#endif
		l^=(((	s0 + 
	...
	p = *CHAN_IN2(uint32_t, key[17], CH(task_set_key, task_encrypt), CH(task_set_key2, task_encrypt));
	l ^= p;
...
void task_start_encrypt() {
	...
	if (n == 0) {
		v0 =((unsigned long)(iv[0]))<<24L;
                v0|=((unsigned long)(iv[1]))<<16L;
                v0|=((unsigned long)(iv[2]))<< 8L;
                v0|=((unsigned long)(iv[3]));
		v1 =((unsigned long)(iv[4]))<<24L;
                v1|=((unsigned long)(iv[5]))<<16L;
                v1|=((unsigned long)(iv[6]))<< 8L;
                v1|=((unsigned long)(iv[7]));
		CHAN_OUT1(uint32_t, input[0], v0, CH(task_start_encrypt, task_encrypt));
		CHAN_OUT1(uint32_t, input[1], v1, CH(task_start_encrypt, task_encrypt));
...
void task_start_encrypt2() {
	...
	unsigned char iv[8];
	unsigned i;
	t = *CHAN_IN1(uint32_t, input[0], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
	iv[0] = (unsigned char)(((t)>>24L)&0xff);
        iv[1] = (unsigned char)(((t)>>16L)&0xff);
        iv[2] = (unsigned char)(((t)>> 8L)&0xff);
        iv[3] = (unsigned char)(((t)     )&0xff);
	t = *CHAN_IN1(uint32_t, input[1], MC_IN_CH(ch_return, task_encrypt, task_set_key2));
	iv[4] = (unsigned char)(((t)>>24L)&0xff);
        iv[5] = (unsigned char)(((t)>>16L)&0xff);
        iv[6] = (unsigned char)(((t)>> 8L)&0xff);
        iv[7] = (unsigned char)(((t)     )&0xff);
	for (i=0; i<8; ++i){
...
void task_start_encrypt3() {
	...
	unsigned n, i;
	uint32_t index2 = *CHAN_IN2(uint32_t, index2, MC_IN_CH(ch_index2, task_init, task_start_encrypt3), SELF_IN_CH(task_start_encrypt3));
	...
	if (index2 == LENGTH) {
		...
		CHAN_OUT1(uint32_t, index2, index2, SELF_CH(task_start_encrypt3));
	       	TRANSITION_TO(task_start_encrypt);	
	}
}
